SELECT TOP (1000)
  ...
  FROM [SQl_SBA.Portfolio].[dbo].[SBA_industsry_standards ]
  where [NAICS Codes] = ''
...
SELECT [NAICS Industry Description],
      substring([NAICS Industry Description],CHARINDEX('â€“',[NAICS Industry Description]) -3,2) as lookup_value,
	  Ltrim(substring([NAICS Industry Description],CHARINDEX('â€“',[NAICS Industry Description]) +3,len([NAICS Industry Description]))) as sector
  FROM [SQl_SBA.Portfolio].[dbo].[SBA_industsry_standards ]
  where [NAICS Codes] = ''
  )
  select * from Cte_lookup
  where lookup_value != ''


  select * 
  into sba_naics_codes_descriprions
  from (
  SELECT [NAICS Industry Description],
      substring([NAICS Industry Description],CHARINDEX('â€“',[NAICS Industry Description]) -3,2) as lookup_value,
	  Ltrim(substring([NAICS Industry Description],CHARINDEX('â€“',[NAICS Industry Description]) +3,len([NAICS Industry Description]))) as sector
  ...
  where [NAICS Codes] = ''
   ...
   SELECT TOP (1000) [NAICS Industry Description]
      ,[lookup_value]
      ,[sector]
  FROM [SQl_SBA.Portfolio].[dbo].[sba_naics_codes_descriprions]

 select replace([NAICS Industry Description],'â€“','-')
 FROM [SQl_SBA.Portfolio].[dbo].[sba_naics_codes_descriprions]

update [SQl_SBA.Portfolio].[dbo].[sba_naics_codes_descriprions]
set [NAICS Industry Description] = replace([NAICS Industry Description],'â€“','-')

update [SQl_SBA.Portfolio].[dbo].[sba_naics_codes_descriprions]
set sector = 'manufacturing'
where lookup_value = 31
